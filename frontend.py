...
forecast_days = st.slider(
    "Select Days for Forecast", min_value=1, max_value=365, value=30
)
    ...
    if response.status_code == 200:
        ...
        combined_data = pd.concat(
            [
                ...
                forecast[["ds", "yhat", "type"]].rename(columns={"yhat": "y"}),
            ]
        )
        ...
        line_chart = (
            ...
            .encode(
                x=alt.X(
                    ...
                    title="Date",
                    axis=alt.Axis(
                        format="%Y-%m-%d", labelFontSize=18, titleFontSize=20
                    ),
                ),
                y=alt.Y(
                    "y:Q",
                ...
                color=alt.Color(
                    "type:N",
                    legend=alt.Legend(
                        title="Data Type", titleFontSize=20, labelFontSize=18
                    ),
                    scale=alt.Scale(
                        domain=["Historical", "Forecast"], range=["blue", "orange"]
                    ),
                ),
